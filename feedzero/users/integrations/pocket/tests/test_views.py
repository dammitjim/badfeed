...
from feedzero.users.integrations.pocket.views import (
    OAuthCallback,
    OAuthEntry,
...
class TestOAuthEntry:
    ...
    def test_sets_state_cookie(self, mocker, auth_client):
        ...
        token_fn = mocker.patch(
            "feedzero.users.integrations.pocket.views.OAuthEntry._get_request_token"
        )
        token_fn.return_value = "123"
        auth_fn = mocker.patch(
            "feedzero.users.integrations.pocket.views.OAuthEntry._get_auth_url"
        )
        auth_fn.return_value = "/my-cool-redirect"
        uuid_fn = mocker.patch("feedzero.users.integrations.pocket.views.uuid4")
...
class TestOAuthCallback:
    ...
    def test_deletes_state_cookie(self, mocker, auth_client):
        ...
        mocker.patch(
            "feedzero.users.integrations.pocket.views.OAuthCallback._set_access_token_to_user"
        )
    ...
    def test_loads_code_from_cookie(self, mocker, auth_client):
        ...
        fn = mocker.patch(
            "feedzero.users.integrations.pocket.views.OAuthCallback._get_access_token"
        )
        fn.return_value = "456"
        auth_client.cookies[PocketConsumerKeyMixin.CODE_COOKIE_KEY] = "123"
        auth_client.get(self.url)
